...
class ModelHandler(object):
	...
	def __init__(self, config):
		...
		elif args.data_name == 'comp':  # comp
			index = list(range(0, len(labels)))
			idx_train, idx_rest, y_train, y_rest = train_test_split(index, labels[0:], stratify=labels[0:],
																	train_size=args.train_ratio, random_state=2, shuffle=True)
			idx_valid, idx_test, y_valid, y_test = train_test_split(idx_rest, y_rest, stratify=y_rest,
																	test_size=args.test_ratio, random_state=2, shuffle=True)
		elif args.data_name == 'AlibabaLoan':  # AlibabaLoan
			index = list(range(0, len(labels)))
			idx_train, idx_rest, y_train, y_rest = train_test_split(index, labels[0:], stratify=labels[0:],
																	train_size=args.train_ratio, random_state=2, shuffle=True)
			idx_valid, idx_test, y_valid, y_test = train_test_split(idx_rest, y_rest, stratify=y_rest,
	...
	def train(self):
		...
		if args.model == 'PCGNN':
			...
			## add intra4 for alibaba
			#inter1 = InterAgg(features, feat_data.shape[1], args.emb_size, self.dataset['train_pos'], adj_lists, [intra1, intra2, intra3, intra4], inter=args.multi_relation, cuda=args.cuda)
		...
		for epoch in range(args.num_epochs):
			# 这里的homo是全局的adj_matrix
			sampled_idx_train = pick_step(idx_train, y_train, self.dataset['homo'], size=len(self.dataset['train_pos'])*2)

			# 将采样得到的训练节点列表进行随机打乱，以去除任何潜在的顺序偏差，这有助于训练过程的随机性和泛化能力。
			random.shuffle(sampled_idx_train)

			# 根据打乱后的训练节点数量和每个批次的大小 (args.batch_size) 来计算需要的批次数量。这里加 1 是为了确保即使不能完全整除时也能包含所有节点。
