...
def load_data(data, prefix='data/'):
	...
	# relation4: only use for alibaba
	relation4 = None
	...
	if data == 'yelp':
		...
		data_file = loadmat(prefix + 'comp.mat')
		labels = data_file['label'].flatten()
		feat_data = data_file['features']#.todense().A
		# load the preprocessed adj_lists
		with open(prefix + 'comp_homo_adjlists.pickle', 'rb') as file:
			homo = pickle.load(file)
		file.close()
		with open(prefix + 'comp_cic_adjlists.pickle', 'rb') as file:
			relation1 = pickle.load(file)
		file.close()
		with open(prefix + 'comp_cpc_adjlists.pickle', 'rb') as file:
			relation2 = pickle.load(file)
		file.close()
		with open(prefix + 'comp_csc_adjlists.pickle', 'rb') as file:
			relation3 = pickle.load(file)
	elif data == 'AlibabaLoan':
		data_file = loadmat(prefix + 'AlibabaLoan.mat')
		labels = data_file['label'].flatten()
		feat_data = data_file['features']#.todense().A
		# load the preprocessed adj_lists
		with open(prefix + 'alibaba_homo_adjlists.pickle', 'rb') as file:
			homo = pickle.load(file)
		file.close()
		with open(prefix + 'alibaba_tat_adjlists.pickle', 'rb') as file:
			relation1 = pickle.load(file)
		file.close()
		with open(prefix + 'alibaba_tgt_adjlists.pickle', 'rb') as file:
			relation2 = pickle.load(file)
		file.close()
		with open(prefix + 'alibaba_tpt_adjlists.pickle', 'rb') as file:
			relation3 = pickle.load(file)
		file.close()
		with open(prefix + 'alibaba_trt_adjlists.pickle', 'rb') as file:
			relation4 = pickle.load(file)

	if relation4 is not None:
		# return for aliababa
		return [homo, relation1, relation2, relation3, relation4], feat_data, labels

	# return for yelpchi and comp
...
def sparse_to_adjlist(sp_matrix, filename):
	...
	# print(sp.eye(sp_matrix.shape[0]).shape)
